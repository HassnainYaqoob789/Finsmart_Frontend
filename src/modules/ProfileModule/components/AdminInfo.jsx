      ...
      <Row align="middle">
        ...
        <Col xs={{ span: 24 }} sm={{ span: 18 }}>
          <Descriptions column={1} size="middle">
            ...
            <Descriptions.Item label={translate('Province')}>{currentAdmin?.province}</Descriptions.Item>
            <Descriptions.Item label={translate('Address')}>{currentAdmin?.address}</Descriptions.Item>
            <Descriptions.Item label={translate('NTN#')}>{currentAdmin?.ntn}</Descriptions.Item>
            ...
            {currentAdmin?.scenarioIds.length > 0 && (
              <Descriptions.Item label='Scenarios'>
                {currentAdmin?.scenarioIds.map((e) => e.scenarioId).join(', ')}
              </Descriptions.Item>
            )}
                {(currentAdmin?.Creds.length > 0 && currentAdmin?.Creds[0]?.sandbox_token) && (
                  <Descriptions.Item label='Sandbox Token'>
                    {currentAdmin?.Creds[0]?.sandbox_token}
                  </Descriptions.Item>
                )}
            {(currentAdmin?.Creds.length > 0 && currentAdmin?.Creds[0]?.production_token) && (
              <Descriptions.Item label='Production Token'>
                {currentAdmin?.Creds[0]?.production_token}
              </Descriptions.Item>
            )}
          </Descriptions>
        </Col>
